...
Student * listuj_studentow_nazwisko_rosnaco(SBaza *b)
{
    for(int j = 0; j < ile_studentow; j++)
    {
        for(int i = 0; i < ile_studentow; i++)
        {
                if(strcmp(Student->nazwisko[i],student->nazwisko[i+1])<0)) // i mniejsze niz i+1
                    {
                        Student->nazwisko[i] = Student->nazwisko[i+1]
                    }
        }
    }
}

Student * listuj_studentow_nazwisko_malejaco(SBaza *b)
{
    for(int j = 0; j < ile_studentow; j++)
    {
        for(int i = 0; i < ile_studentow; i++)
        {
                if(strcmp(Student->nazwisko[i],student->nazwisko[i+1])>0))
                    {
                        Student->nazwisko[i] = Student->nazwisko[i+1]
                    }
        }
...
Przedmiot *przedmioty_sortowane_po_nazwie(SBaza *baza);
{
    for(int j = 0; j < ile_przedmiotow; j++)
    {
        ...
        {
                if(strcmp(Przedmiot->nazwa[i],Przedmiot->nazwa[i+1])<0))
                    {
                        Przedmiot->nazwa[i] = Przedmiot->nazwa[i+1]
                    }
        }
    }
}
...
Przedmiot *przedmioty_sortowane_po_kodzie(SBaza *baza)
{
    for(int j = 0; j < ile_przedmiotow; j++)
    {
        for(int i = 0; i < ile_przedmiotow; i++)
        {
                if(strcmp(Przedmiot->kod_przed[i],Przedmiot->kod_przed[i+1])<0))
                    {
                        Przedmiot->kod_przed[i] = Przedmiot->kod_przed[i+1]
                    }
        }
    }
}
...
Przedmiot *przedmiot_o_zadanym_kodzie(SBaza *baza)
{
    for(int i = 0; i < ile_przedmiotow; i++)
    {
        if(Przedmiot->kod_przed[i])
        {
            printf("%s", kod_przed[i]);
        }
    }
}
...
Student *oceny_danego_studenta(SBaza *baza)
{
    for(int i = 0; i < ile_studentow; i++)
    {
        if(Student[i]->nr_albumu)
        {
            for(int j = 0; i < ile_przedmiotow; j++)
            {
                printf("%d", Przedmiot[j]->Ocena);
            }
        }
    }
}